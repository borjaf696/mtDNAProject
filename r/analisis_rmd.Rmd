...
data_filtered$Rounded.Log.GB.Seqs <- as.numeric(floor(data_filtered$Rounded.Log.GB.Seqs))
for (a in l_a){
  # Eliminamos datos fuera de lugar
  lower_bound <- quantile(y, 0.00);lower_bound
  upper_bound <- quantile(y, 1 - a);upper_bound
  ...
  dPIG <- histDist(data_tmp$Rounded.Log.GB.Seqs, "PIG",
                   main = "PIG", trace = FALSE,xlim=c(0,20))   # Poisson inverse gaussian
  dSI <- histDist(data_tmp$Rounded.Log.GB.Seqs, "SICHEL",
                  main = "SICHEL", trace = FALSE, xlim=c(0,20)) # Sichel? Inverse Gaussian distribution
  ...
  #windows()
  par(mfrow = c(1,1))
  plot(data_tmp$CVTOT,data_tmp$Rounded.Log.GB.Seqs,pch=16,ylim=c(0,200), xlab="CVTOT",
      ylab="Appearances",main=paste("Filtro: ",a))
  lines(x,mL.p,col="red",type="l",lwd=2)
  lines(x,mC.p,col="blue",type="l",lwd=2)
  lines(x,mS.p,col="orange",type="l",lwd=2)
  ...
  ## En base a esto vamos a evaluar los resultados via plot - PIG
  # Lineal + ajuste de sd
  mL <- gamlss(Rounded.Log.GB.Seqs~CVTOT , data=na.omit(data_tmp), family = PIG,
               sigma.fo=~CVTOT, n.cyc = 5, trace = FALSE)
  # Cuadrático + ajuste de sd
  mC <- gamlss(Rounded.Log.GB.Seqs~CVTOT+I(CVTOT^2) , data=na.omit(data_tmp), family = PIG,
               sigma.fo=~CVTOT, n.cyc = 5, trace = FALSE)
  # Vía smoothing spline + ajuste de sd
  mS <- gamlss(Rounded.Log.GB.Seqs~cs(CVTOT) , data=na.omit(data_tmp), family = PIG,
               sigma.fo=~CVTOT, n.cyc = 5, trace = FALSE)
