...
```{r}
data <- na.omit(read.csv('../output/lsu_df_cancer.csv', sep = ',', dec = '.', header = T))
dim(data)
head(data)
data$CVTOT <- as.numeric(as.character(data$CVTOT))
data$GB.Seqs <- na.omit(as.numeric(as.character(data$GB.Seqs)))
data$GB.Seqs.Corrected <- na.omit(as.numeric(as.character(data$GB.Seqs)))
# Corregimos el impacto del haplogrupo:
data$GB.Seqs.Original <- data$GB.Seqs
data$GB.Seqs <- data$GB.Seqs
...
# Haplogrupos diferentes - 6- 36
#data_filtered$distinct_haplogroups <- apply(data_filtered[6:36], 1,function(x) sum(x > 0))
...
Breve análisis acerca de que posiciones presentar mayor número de variaciones corregidas y cuales mayor número de haplogrupos distintos (estudios al 90\% y al 95\%).
...
for (a in alphas)
{
  ...
  outlier_ind <- which(y > upper_bound)
  data_tmp <- data_filtered[outlier_ind,]
  write.csv(data_tmp, paste("../output_r/lsu_",a,".csv"), sep = ',')
}

### Residuos con variacion
length(unique(data[data$GB.Seqs.Original!=0,]$Genomic))
length(unique(data[data$GB.Seqs.Original!=0,]$Genomic))/1559
## Total
sum(data$GB.Seqs.Original)
87136/1559

## Maximo
max(data$GB.Seqs.Original)
data[data$GB.Seqs.Original == max(data$GB.Seqs.Original),]$Genomic
# Variantes en regiones
length(unique(data[(data$CVTOT >= 0) & (data$CVTOT < 1),]$Genomic))
length(unique(data[(data$CVTOT >= 1) & (data$CVTOT < 1.5) ,]$Genomic))
length(unique(data[(data$CVTOT >= 1.5) & (data$CVTOT < 1.9) ,]$Genomic))
length(unique(data[(data$CVTOT >= 1.9),]$Genomic))

# Total variaciones
sum(na.omit(data[(data$CVTOT <0),]$GB.Seqs.Original))
sum(na.omit(data[(data$CVTOT >= 0) & (data$CVTOT < 1),]$GB.Seqs.Original))
sum(na.omit(data[(data$CVTOT >= 1) & (data$CVTOT < 1.5),]$GB.Seqs.Original))
sum(na.omit(data[(data$CVTOT >= 1.5) & (data$CVTOT < 1.9),]$GB.Seqs.Original))
sum(na.omit(data[(data$CVTOT >= 1.9),]$GB.Seqs.Original))

# Numero de sitios con variacion
length(unique(na.omit(data[(data$CVTOT < 0),])$Genomic))
length(unique(na.omit(data[(data$CVTOT >= 0) & (data$CVTOT < 1) & (data$GB.Seqs!= 0),])$Genomic))
length(unique(na.omit(data[(data$CVTOT >= 1) & (data$CVTOT < 1.5) & (data$GB.Seqs!= 0),])$Genomic))
length(unique(na.omit(data[(data$CVTOT >= 1.5) & (data$CVTOT < 1.9) & (data$GB.Seqs!= 0),])$Genomic))
length(unique(na.omit(data[(data$CVTOT >= 1.9) & (data$GB.Seqs!= 0),])$Genomic))
```
...
for (a in l_a){
  ...
  dDel <- histDist(data_tmp$GB.Seqs, "DEL",
                    main = "DEL", trace = FALSE, xlim=c(0,20)) # Zero inf. pois. 2
  par(op)
  print(AIC(dPO, dNBI, dPIG, dSI, dZIP, dZIP2, dDel))
  ...
  ## Igual que se visualizaba anteriormente, los mejores resultados vienen de PIG, DEL y SICHEL (Igual
  ...
  ##  DEL - pero desafortunadamente este resultado es bastante poco ajustado debido a que se fija unicamente a la zona inferior
  ##  Ajustamos también PIG
  ...
  # Lineal + ajuste de sd
  mL <- gamlss(GB.Seqs~CVTOT , data=na.omit(data_tmp), family =DEL,
               sigma.fo=~CVTOT, n.cyc = 5, trace = FALSE)
  # Cuadrático + ajuste de sd
  mC <- gamlss(GB.Seqs~CVTOT+I(CVTOT^2) , data=na.omit(data_tmp), family =DEL,
               sigma.fo=~CVTOT, n.cyc = 5, trace = FALSE)
  # Vía smoothing spline + ajuste de sd
  mS <- gamlss(GB.Seqs~cs(CVTOT) , data=na.omit(data_tmp), family =DEL,
               sigma.fo=~CVTOT, n.cyc = 5, trace = FALSE)
  ...
  legend("topright", c("DEL.Lineal","DEL.Cuadrático","DEL.Splines"),
         lty=c(2,2,2),col=c("red","blue","orange"), ncol=1, lwd=rep(2,3), inset = .05, cex=0.75)
  
  ## En base a esto vamos a evaluar los resultados via plot - PIG
  # Lineal + ajuste de sd
...
for (i in 1:(L-1))
  {
  q_pos <- rq(GB.Seqs~ CVTOT, data=data_filtered,tau=quant[L-i] ,method="br")
  q_neg <- rq(GB.Seqs~ CVTOT, data=data_filtered,tau=quant[L+i] ,method="br")
  abline(q_pos,col=colores[i], lty=1, lwd =2)
  abline(q_neg,col=colores[i], lty=1,lwd =2 )
  }
...
splines_adjust<-function(DataSub,columna_y, columna_x, df,m,alpha,B,mainT,columnay = 'Apariciones', type = 'regular') {
  ...
  if (type == 'regular'){
	  spl<-gam(y_l~s(x_l, bs = "cr", fx = TRUE))
  }
  if (type == 'b')
  {
  ...
  print(AIC(spl))
    ...
    y_loc <- y_loc[ind_x]
		# Fit spline to data, with cross-validation to pick lambda
		if (type == 'regular'){
		  spl<-gam(y_loc~s(x_loc, bs = "cr", fx = TRUE))
		}  
		if (type == 'b'){
...
for (a in l_a)
{
  ...
  for (df in dfs)
  {
    splines_adjust(data_tmp, 'Log.GB.Seqs', 'CVTOT', df,300,0.025,100,paste("Smoothing Spline Regression ",df,'(dfs) Filtro = ',a), columnay = 'Log(Apariciones +1)')
...
```{r echo=FALSE, warning=FALSE, =FALSE}
...
for (a in l_a){
  ...
  for (df in dfs)
  {
    spls[i] <- splines_adjust(data_tmp, 'Log.GB.Seqs', 'CVTOT', df,6,0.5,1000,paste("P-Spline Regression ",df,'(K) alpha = ',a), type = 'p')
    i <- i + 1
  }
}
```
...
lo <- gamlss(y_sort~lo(~x_sort), family = 'BI')
...
```{r eval=FALSE, include=FALSE}
breaks <- seq(-1, 2, by = 0.125)
# Histogramas
apariciones.media <- numeric(length(breaks)-1)
log.apariciones.media <- numeric(length(breaks) - 1)
for (i in seq(length(breaks)-1))
{
  apariciones.media[i] <- sum(data_filtered$GB.Seqs[data_filtered$CVTOT >= breaks[i] & data_filtered$CVTOT < breaks[i+1]])/length(data_filtered$GB.Seqs[data_filtered$CVTOT >= breaks[i] & data_filtered$CVTOT < breaks[i+1]])
  log.apariciones.media[i] <- sum(data_filtered$Log.GB.Seqs[data_filtered$CVTOT >= breaks[i] & data_filtered$CVTOT < breaks[i+1]])/length(data_filtered$GB.Seqs[data_filtered$CVTOT >= breaks[i] & data_filtered$CVTOT < breaks[i+1]])
}
par(mfrow=c(1,2))
...
Variación asociada a un haplogrupo e individuo pertenece a dicho haplogrupo - eliminar (sacar las mismas gráficas) - Incorporado.

Cambiar la base de datos para coger solo de humano y quitar lo de EColi. Ya incorporado en los análisis superiores

## 05/11/2020

Tratar de incluir la información de los haplogrupos a modo de nueva variable:

  * Hasta el momento se han corregido las apariciones en base a si una variación X está asociada a un haplogrupo y adicionalmente el individuo que posee dicha variación pertenece a dicho haplogrupo.
  * Probar a ubicar los haplogrupos distintos que no han sido asociados a ningún haplogrupo en la variantes. En principio, una variación que aparezca en múltiples haplogrupos no asociados puede aportar información acerca del número de ocurrencias y lo mismo para el caso donde una el número de haplogrupos es bajo. (uno indicaría mucha presencia de la variación mientas que la otra probablemente muchas secuenciaciones del haplogrupo)
  
  Los modelos a ajustar serán:
    
    * Modelo aditivo con variables CVIndex únicamente a modo de control.
    * Modelo aditivo con variables CVIndex y haplogrupos_distintos, y se incorporarán dos versiones con y sin interacción.

```{r eval=FALSE, include=FALSE}
md_control <- gam(Log.GB.Seqs~s(CVTOT), data = data_filtered)
par(mfrow = c(1,4))
gam.check(md_control)
summary(md_control)
par(mfrow = c(1,1))
plot(md_control,shade=T,residuals=T,cex=4)
```

En base a los resultados obtenidos se puede decir que a nivel de ajuste de datos la variable CVTOT presenta significancia clara. Además en base al gam.check podemos "asegurar" que el número de grados de libertad dados a la variable no se encuentra en ningún caso infraestimado. Por otro lado, la deviance es ínfima lo que tiene sentido debido a la enorme dispersión que presentan los datos iniciales. Pero dado que a nosotros solamente nos interesa capturar la tendencia de los datos y no realmente ajustar un modelo para hacer predicciones continuamos hacia adelante.

```{r eval=FALSE, include=FALSE}
md_adjust_1 <- gam(Log.GB.Seqs~s(CVTOT) + s(distinct_haplogroups), data = data_filtered)
par(mfrow = c(1,4))
gam.check(md_adjust_1)
summary(md_adjust_1)
par(mfrow = c(1,2))
plot(md_adjust_1,shade=T,residuals=T,cex=4)
vis.gam(md_adjust_1, plot.type = "contour")
```

Obviamente al incorporar la variable "distinct.haplogroups" la cantidad de variabilidad explicada por el modelo crece enormemente, pero es más debido a la correlación (clara) existenten entre la variable regresora y la variable a regresada (a más secuencias en asociadas a la variación mayor número esperado de haplogrupos asociado). En base a eso podemos asegurar que no existe comportamiento "caracterizable" de atípico sino que se sigue una pauta lógica.

Por contrastar la existencia de interacción entre CVIndex y los haplogrupos distintos:

```{r eval=FALSE, include=FALSE}
md_adjust_1 <- gam(Log.GB.Seqs~s(CVTOT) + s(distinct_haplogroups) + s(CVTOT, distinct_haplogroups), data = data_filtered)
par(mfrow = c(1,4))
gam.check(md_adjust_1)
summary(md_adjust_1)
par(mfrow = c(1,2))
...
A la vista de los resultados habría que descartar la interacción entre las variables regresoras como elemento que ayude a explicar el número de secuencias.