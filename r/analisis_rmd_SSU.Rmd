...
for (a in alphas)
{
  ...
  write.csv(data_tmp, paste("../output_r/ssu_",a,".csv"), sep = ',')
}

### Residuos con variacion
length(unique(data[data$GB.Seqs.Original!=0,]$Genomic))
length(unique(data[data$GB.Seqs.Original!=0,]$Genomic))/954
## Total
sum(data$GB.Seqs.Original)
sum(data$GB.Seqs.Original)/954

## Maximo
max(data$GB.Seqs)
data[data$GB.Seqs == max(data$GB.Seqs),]$Genomic
# Variantes en regiones
length(unique(data[(data$CVTOT < 0),]$Genomic))
...
length(unique(na.omit(data[(data$CVTOT < 0) & (data$GB.Seqs != 0),])$Genomic))
...
for (a in l_a){
  ...
  # Lineal + ajuste de sd
  mL <- gamlss(GB.Seqs~CVTOT , data=na.omit(data_tmp), family =SICHEL,
               sigma.fo=~CVTOT, n.cyc = 5, trace = FALSE)
  # Cuadrático + ajuste de sd
  mC <- gamlss(GB.Seqs~CVTOT+I(CVTOT^2) , data=na.omit(data_tmp), family = SICHEL,
               sigma.fo=~CVTOT, n.cyc = 5, trace = FALSE)
  # Vía smoothing spline + ajuste de sd
  mS <- gamlss(GB.Seqs~cs(CVTOT) , data=na.omit(data_tmp), family = SICHEL,
               sigma.fo=~CVTOT, n.cyc = 5, trace = FALSE)
  ...
  legend("topright", c("SICHEL.Lineal","SICHEL.Cuadrático","SICHEL.Splines"),
...
for (i in 1:(L-1))
{
  rq_i <- rq(GB.Seqs~ CVTOT, data=data_filtered,tau=quant[L-i] ,method="br", alpha = .05, ci = T)
  rq_i2 <- rq(GB.Seqs~ CVTOT, data=data_filtered,tau=quant[L+i] ,method="br", alpha = .05, ci = T)
  abline(rq_i,col=colores[i], lty=1, lwd =2)
  abline(rq_i2,col=colores[i], lty=1,lwd =2 )
}
...
hist(data_filtered$GB.Seqs[data_filtered$GB.Seqs > 1], breaks = 'fd', freq = F, main = 'Histograma GB.Seqs > 0')
hist(data_filtered$GB.Seqs[data_filtered$GB.Seqs > 2], breaks = 'fd', freq = F, main = 'Histograma original > 1')
data_filtered$Bin.GB.Seqs[data_filtered$GB.Seqs > 1] <- 1
